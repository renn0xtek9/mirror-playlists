...
def get_all_playlist_files(playlist_root: Path) -> List[Path]:
    """List all .m3u files stored under the playlist root path.

    Args:
        playlist_root: Path: path of where all files are located
    Returns:
        List[Path]: list of path to all files
    """
    playlist_files = list(playlist_root.glob("**/*.m3u"))
...
def create_destination_file(source_song_path: Path, music_folder: Path, destination: Path) -> Path:
    ...
    Example:
        ...
        music_folder=/home/foo/Music
        destination=/mnt/music will point
    The function will return /mnt/music/Artist/bar.mp3
    Args:
        source_song_path(Path): path of the song
        music_folder(Path): path of the root of music collection
        destination(Path): path of mirroring destination
    Return:
        Path: destination of song path
    """
    relative_path = source_song_path.relative_to(music_folder)
...
def get_relative_path_to_song_from_playlist_file(playlist_file: Path, song_file: Path) -> str:
    """Get the reatlive path to the song from the playlist file.

    Example with
    playlist_file = /home/foo/music/Playlists/myplaylist.m3u and
    song_file = /home/foo/music/bar.mp3
    the function will return ../bar.mp3
    Args:
        playlist_file (Path): path of the playlist file that we want to mirror
        song_file (Path): path to the song file
    Return:
        str: relative path to reacht the song file from the playlist file
    """
    if playlist_file == song_file:
        raise ValueError("Song file and playlist file identical {song_file}")
    playlist_file_path_list = list(playlist_file.parts)
    song_file_path_list = list(song_file.parts)
...
def get_new_content_of_playlist_file(playlist_file: Path) -> List[str]:
    ...
    for song_path in list_of_song_path:
        content.append(get_relative_path_to_song_from_playlist_file(playlist_file, song_path))
    return content


def get_destination_path_of_playlist_file(
    music_folder: Path, playlist_file_path: Path, destination_folder: Path
) -> Path:
    ...
    Example:
        ...
        destination_folder = /mnt/Music
    The function will return /mnt/Music/Playlists/playlist.m3u
    Args:
        music_folder (Path): root folder of the music repository to be mirror
        playlist_file_path (Path): path of the playlist file
        destination_folder: Path: destination where we should mirror files
    Return
        Path: path of the playlist file where the new playlist shall be saved
    """
    relative_path = playlist_file_path.relative_to(music_folder)
    return destination_folder / relative_path
...
def mirror_all_playlist(music_folder: Path, playlist_root: Path, destination_folder: Path) -> None:
    """Mirror all playlist and there content to the given destination.

    Args:
        music_folder: Path: root folder of the music repository to be mirror
        playlist_root: Path: root folder of the playlist files
        destination_folder: Path: destination where we should mirror files
    Raises:
    ...
    """
    if not is_folder_existing(music_folder):
        raise FileNotFoundError("Music root folder not existing {music_folder}")
    if not is_folder_existing(playlist_root):
        raise FileNotFoundError("Playlist root folder not existing {playlist_root}")
    if not is_folder_existing(destination_folder):
        raise FileNotFoundError("Destination fodler not existing {destination_folder}")
    if not os.access(str(destination_folder), os.W_OK):
        raise PermissionError("No write access to {destination_folder}")

    playlist_files = get_all_playlist_files(playlist_root)
    for playlist_file in playlist_files:
        logging.info("Mirroring: %s", str(playlist_file))
        list_of_song_path = parse_playlist(playlist_file)
        for song_path in list_of_song_path:
            destination_path = create_destination_file(song_path, music_folder, destination_folder)
        ...
        new_playlist_file_path = get_destination_path_of_playlist_file(music_folder, playlist_file, destination_folder)
